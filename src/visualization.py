...
def plot_proteingym(df_total, run_name, epoch):
    """
    绘制ProteinGym评估结果

    参数:
    df_total (pd.DataFrame): 包含所有蛋白质评估结果的数据框
    run_name (str): 运行名称标识符
    epoch (int): 模型训练epoch数
    """

    if 'SSEmb' not in df_total.columns and 'score_ml' in df_total.columns:
        df_total['SSEmb'] = df_total['score_ml']
    # 1. 数据清洗和预处理
    # 确保分数列是数值类型
    df_total['score_dms'] = pd.to_numeric(df_total['score_dms'], errors='coerce')
    df_total['score_ml'] = pd.to_numeric(df_total['score_ml'], errors='coerce')

    # 移除无效值
    df_total = df_total.dropna(subset=['score_dms', 'score_ml'])
    ...
    # 2. 加载验证集排除列表
    try:
        with open("../data/test/proteingym/val_list.pkl", "rb") as fp:
            benchmark_dms_exclude_list = pickle.load(fp)
    except FileNotFoundError:
        print("未找到val_list.pkl文件，将处理所有蛋白质")
        benchmark_dms_exclude_list = []

    # 3. 计算每个蛋白质的相关性
    # 重命名列以匹配后续处理
    df = df_total.rename(columns={"dms_id": "DMS_id"})

    # 排除验证集蛋白质
    df_benchmark = df[~df["DMS_id"].isin(benchmark_dms_exclude_list)]

    # 计算Spearman相关性
    corr_obj = (
        df_benchmark.groupby("DMS_id")[["score_ml", "score_dms"]]
    ...
    # 创建相关性数据框
    df_corrs = pd.DataFrame(corrs.items(), columns=["DMS_id", "SSEmb"])
    ...
    # 加载 ProteinGym UniProt-level 基准数据
    ...
    for col in numeric_cols:
        ...
        if df_benchmark[col].isnull().any():
            df_benchmark[col] = df_benchmark[col].apply(
                lambda x: float(re.search(r"[-+]?\d*\.\d+|\d+", str(x)).group())
                if pd.notnull(x) and re.search(r"[-+]?\d*\.\d+|\d+", str(x))
                else np.nan
            )
