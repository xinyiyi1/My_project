...
class ProteinGraphData(data.Dataset):
    ...
    def _featurize_as_graph(self, protein):
        # 确保msa字段存在
        if "msa" not in protein:
            raise KeyError(f"蛋白质 {protein['name']} 缺失msa字段")
        if "esm2_embeddings" not in protein:
            raise KeyError(
                f"蛋白质 {protein.get('name', '未知名称')} 缺失ESM2嵌入。"
                f"现有字段: {list(protein.keys())}"
            )

        seq = protein['seq']
        coords = protein["coords"]
        assert len(seq) == len(coords), f"Data integrity failed: {protein['name']} seq-coords length mismatch"

        # 查看 ESM 长度和序列长度
        esm_array = np.asarray(protein["esm2_embeddings"], dtype=np.float32)
        print(f"Protein {protein['name']} - ESM length: {esm_array.shape[0]}, Seq length: {len(seq)}")
        ...
        # 确保 ESM 嵌入长度与序列长度一致
        assert esm_array.shape[0] == len(seq_clean), f"ESM dim mismatch in {protein['name']}"

        name = protein['name']
        with torch.no_grad():
            coords = torch.tensor(np.array(protein["coords"]).astype(np.float32), device=self.device)
            seq = torch.as_tensor([self.letter_to_num[a] for a in seq_clean],
            ...
            seq_len = seq.size()[0]
            dist_mask_np = np.zeros((seq_len, seq_len), dtype=bool)

            # 填充边缘索引
            edge_rows = edge_index[1, :].cpu().numpy()
            edge_cols = edge_index[0, :].cpu().numpy()
            dist_mask_np[edge_rows, edge_cols] = True

            # 填充对角线
            diag_indices = np.diag_indices(seq_len)
            dist_mask_np[diag_indices] = True

            # 转换为Tensor并取反
            dist_mask = torch.from_numpy(dist_mask_np).to(seq.device)
            dist_mask = ~dist_mask
            ...
            esm2_feats = torch.tensor(protein["esm2_embeddings"], dtype=torch.float32)
